...
async def init_db(db_path: Path | None = None) -> None:
    ...
    async with aiosqlite.connect(str(db_path)) as conn:
        ...
        for column in ("ticket_status", "ticket_assignee", "ticket_tags"):
        ...
        await conn.execute(
            """INSERT INTO parsed_messages
               (message_id, ticket_id, event_type, author_type, author_email,
                channel, customer_email, ticket_subject, ticket_status, ticket_assignee, ticket_tags, message_text, intents,
            ...
            (message_id, event["ticket_id"], event["event_type"], event["author_type"],
             ...
             json.dumps(event.get("ticket_tags") or []), event.get("message_text"), intent_names,
...
async def record_parsed_message(
    ...
    await db.execute(
           ...
           (message_id, ticket_id, event_type, author_type,
            author_email, channel, customer_email, ticket_subject, ticket_status, ticket_assignee, ticket_tags,
