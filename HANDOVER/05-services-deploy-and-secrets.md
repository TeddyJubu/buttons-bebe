...
| `buttonsbebe-whatsapp-connect` | `/bin/bash -c 'exec __NODE__ ".../whatsapp-connect/server.js"'` (WorkingDirectory `.../whatsapp-connect`) | `WA_PORT=8085`, `WA_TOKEN=__WA_TOKEN__`, `WA_PASSWORD=<hardcoded default>`, `WA_AUTH_DIR=.../whatsapp-connect/auth`, `HERMES_BIN=__HERMES_BIN__` | `whatsapp-connect/buttonsbebe-whatsapp-connect.service` |
...
> **`__PLACEHOLDER__` substitution:** the WhatsApp and KB‑admin unit files ship with `__NODE__`, `__WA_TOKEN__`, `__HERMES_BIN__` tokens. These are substituted with real absolute paths/values at deploy time on the VPS (Node lives at a non‑fixed path; the `Buttonsbebe Agent` folder name contains a space). If you re‑deploy, you must fill these in.
>
...
| `WA_TOKEN` | MAIN `.env` + `whatsapp-connect` unit (`__WA_TOKEN__`) | WhatsApp connect (`:8085`) | Secret path segment for the QR pairing URL `/connect-whatsapp/<token>/` |
| `WA_PASSWORD` | MAIN `.env` + `whatsapp-connect` unit | WhatsApp connect (`:8085`) | Gate on the connect page. ⚠️ **The committed unit file hardcodes a weak default — rotate it and move to env (see §5).** |
...
4. **Hardcoded WhatsApp password.** `whatsapp-connect/buttonsbebe-whatsapp-connect.service` sets `WA_PASSWORD` to a **weak hardcoded default** committed in the repo. Rotate it and source it from env before/at handover.
