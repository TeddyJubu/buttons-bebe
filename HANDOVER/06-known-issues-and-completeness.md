...
| WhatsApp escalation channel + owner QR pairing + 2-way bridge | **LIVE** (added 2026-07-07) | `whatsapp-connect/server.js` (:8085), `processor/whatsapp_notifier.py` | Fires only when a ticket is classified IMMEDIATE — which today only Hermes can do (see classifier stub). |
...
| **`feedback_collector.py` — poll-based reply capture** | **RETIRED / FAIL-CLOSED** | `processor/feedback_collector.py`, `feedback/collector.py` | Superseded by the Console-action capture (`learning.py` + nightly promotion). Processor hook, Hermes helper, and standalone poll command refuse to run unless `FEEDBACK_LEGACY_OPT_IN=1` is explicitly set for a bounded rollback test. No timer. |
...
|---|---|---|---|---|---|
| **#12** | **High** | Security | **Fixed in repository; VPS rollout pending.** WhatsApp Connect now refuses missing/placeholder secrets, uses a separate `WA_SEND_SECRET`, authenticates and audit-logs `/send`, and reads only its dedicated env file. The live VPS must receive three rotated secrets and the coupled caller/receiver configuration before restart. | `whatsapp-connect/server.js`, `security.js`, `.service`; `processor/whatsapp_notifier.py` | Generate unique `WA_TOKEN`, `WA_PASSWORD`, `WA_SEND_SECRET` in `whatsapp-connect/.env`; deploy the processor Bearer header and matching secret drop-in; verify unauthenticated POST returns 401 and a valid test alert succeeds. |
...
| **#4** | Low | Learning | The old poll-based `feedback_collector.py` was a **STUB (logs only)** and is superseded by the LIVE Console-action capture (`learning.py` + nightly promotion) per `CLAUDE.md` §8. | `processor/feedback_collector.py`, `feedback/collector.py` | **Retired / fail-closed.** Keep no timer; use `FEEDBACK_LEGACY_OPT_IN=1` only for a bounded rollback test. |
...
| **L4** | Low | Config | The root examples document shared Gorgias + Shopify settings while webhook-specific `WEBHOOK_SECRET`, `WHATSAPP_SEND_URL`, and `WA_SEND_SECRET` live in service-specific configuration. A duplicate `env.example` (no dot) also exists at repo root. | `.env.example`, `env.example`, `webhook/.env.example` | Keep the examples aligned without placing secrets in the shared application environment. |
