...
class ProjectionTests(unittest.TestCase):
    def setUp(self):
        ...
        with sqlite3.connect(self.source) as db:
            db.execute('CREATE TABLE parsed_messages(ticket_id INTEGER,message_id TEXT,author_type TEXT,author_email TEXT,customer_email TEXT,ticket_subject TEXT,channel TEXT,ticket_status TEXT,ticket_assignee TEXT,ticket_tags TEXT,ticket_priority TEXT,created_at TEXT,received_at TEXT,is_customer_message INTEGER,message_text TEXT)')
            ...
            db.execute("INSERT INTO parsed_messages VALUES(1,'m1','customer','qa@example.com','qa@example.com','<script>title</script>','email','closed','agent@example.com','[\"vip\"]','urgent','2099-01-01','2099-01-01',1,?)",('<img onerror=alert(1)>'+('x'*21000),))
    ...
    def test_latest_event_does_not_inherit_prior_customer_identity(self):
        with sqlite3.connect(self.source) as db:
            ...
            db.execute("INSERT INTO parsed_messages VALUES(1,'m2','customer','new@example.com','new@example.com','New message','email',NULL,NULL,NULL,NULL,'2099-02-01','2099-02-01',1,'Hello')")
    ...
    def test_observed_status_uses_latest_event(self):
        with sqlite3.connect(self.source) as db:
            db.execute("INSERT INTO parsed_messages VALUES(1,'m3','customer','new@example.com','new@example.com','New message','sms','snoozed','agent@example.com','[\"vip\",\"urgent\"]','low','2099-03-01','2099-03-01',1,'Hello')")
        export(self.source,self.dest,now=self.now)
        ticket=query('helpdesk.get_ticket',{'ticketId':'gorgias:1'},self.dest)['ticket']
        ...
        with sqlite3.connect(self.source) as db:
            db.execute("INSERT INTO parsed_messages VALUES(1,'m4','customer','new@example.com','new@example.com','New message','sms',NULL,NULL,NULL,NULL,'2099-04-01','2099-04-01',1,'Hello')")
        export(self.source,self.dest,now=self.now)
        ticket=query('helpdesk.get_ticket',{'ticketId':'gorgias:1'},self.dest)['ticket']
    ...
    def test_ticket_and_message_windows_are_bounded(self):
        with sqlite3.connect(self.source) as db:
            for number in range(2,503):
                db.execute("INSERT INTO parsed_messages VALUES(?,?,'customer','','','Subject','email',NULL,NULL,NULL,NULL,'2099-01-01','2099-01-01',1,'Hello')",(number,f'm{number}'))
            for number in range(102):
                db.execute("INSERT INTO parsed_messages VALUES(1,?,'customer','','','Subject','email',NULL,NULL,NULL,NULL,'2099-02-01','2099-02-01',1,'Hello')",(f'extra{number}',))
    ...
    def test_draft_lineage_withholds_superseded_customer_reply(self):
        with sqlite3.connect(self.source) as db:
            db.execute("INSERT INTO parsed_messages VALUES(1,'newer','customer','','','Followup','email',NULL,NULL,NULL,NULL,'2099-03-01','2099-03-01',1,'New question')")
        export(self.source,self.dest,now=self.now)
        ticket=query('helpdesk.get_ticket',{'ticketId':'gorgias:1'},self.dest)['ticket']
