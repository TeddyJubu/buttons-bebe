...
assert(await page.locator('.composer [data-action="copy-reply"]').isVisible());
assert(await page.locator('.composer [data-action="copy-reply"]').evaluate(el=>el.classList.contains('primary')));
assert.equal(await page.locator('.composer [data-action="send-gate"]').count(),0);
const compactHeight=await page.locator('#reply').evaluate(el=>el.getBoundingClientRect().height);
await page.locator('#reply').fill(Array.from({length:12},(_,index)=>`Reply line ${index+1}`).join('\n'));
const expandedHeight=await page.locator('#reply').evaluate(el=>el.getBoundingClientRect().height);
assert(expandedHeight>compactHeight,`reply editor did not grow: ${compactHeight} → ${expandedHeight}`);
await page.locator('#reply').fill('Short reply');
assert((await page.locator('#reply').evaluate(el=>el.getBoundingClientRect().height))<expandedHeight);
...
console.log('Passed: 11 viewport widths, independent scrolling, compact auto-growing composer, copy-only action, state colors, filters/search, local draft persistence, tabs/keyboard, rail collapse/drawer/focus, reduced motion, zoom layout, empty/error/auth states; zero browser errors.');
await browser.close();
