...
- Preserve the Inbox API and MCP tools' read-only provider boundary. The owner-authorized Inbox Read & write switch obtains a page-scoped, session-bound grant through the authenticated console API. A person must review and confirm each reply; switching off, expiry, or reload returns the page to read-only. See console-src/inbox2/README.md and webhook/src/bb_webhook/inbox_send_access.py. No Hermes, Inbox service, or Shopify rail process may send or gain an implicit provider write path.
